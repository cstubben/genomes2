   ...
   if(ncol(x) != 23){
      ...
      prj <- x
       ...
       names(x) <- c("name", "taxid", "bioproject", "pid", "group", 
            "subgroup", "size", "gc", "refseq", "insdc", "plasmid.refseq", 
            "plasmid.insdc", "wgs", "scaffolds", "genes", "proteins", 
            "released", "modified", "status", "center", "biosample", "assembly", "reference" )
        x$released <- as.Date(x$released)
        x$modified <- as.Date(x$modified)

      prj <- x[order(x$name), c(4,1,19,17, 2, 3, 5:16,18, 20:23)]
      rownames(prj) <- NULL
